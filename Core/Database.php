...
public function query($query, $params = [])
{
    ...
    try {
        ...
        // Bind params with appropriate PDO types
        foreach ($params as $key => $value) {
            // Support both ':name' and 'name'
            $paramKey = (is_string($key) && str_starts_with($key, ':')) ? $key : ':' . $key;

            // Treat pure-digit strings as ints too (common from $_GET)
            if (is_int($value) || (is_string($value) && ctype_digit($value))) {
                $this->statement->bindValue($paramKey, (int) $value, PDO::PARAM_INT);
            } elseif (is_bool($value)) {
                $this->statement->bindValue($paramKey, $value ? 1 : 0, PDO::PARAM_INT);
            } elseif ($value === null) {
                $this->statement->bindValue($paramKey, null, PDO::PARAM_NULL);
            } else {
                $this->statement->bindValue($paramKey, $value, PDO::PARAM_STR);
            }
        }
    ...
    public function fetchColumn($query, $params = [])
    {
        try {
            ...
            return $result !== false ? (float)$result : 0.00; // Ensure it returns a valid number
        } catch (\PDOException $e) {
